...
Интерактивный Ctrl+C не является привилегированным «сигналом ядра». Shell и
`libvarania` передают `sessiond` ослабленный `CAP_CONTROL`, terminal отправляет
`SESSION_INTERRUPT`, а sessiond вызывает `PROCESS_KILL` со status 130. Поэтому
путь прерывания не зависит от кода или состояния foreground-программы.
